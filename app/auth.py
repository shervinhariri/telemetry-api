...
def _extract_token(req: Request) -> str | None:
    ...
    if auth:
        ...
        if len(parts) == 2 and parts[0].lower() == "bearer":
            return parts[1]
        # also allow raw "Authorization: <token>"
        if len(parts) == 1:
            return parts[0]
    # X-API-Key
...
def require_key(req: Request, db: Session = Depends(get_db)) -> ApiKey:
    ...
    # attach normalized scopes for downstream checks
    key._norm_scopes = _normalize_scopes(key.scopes)
    return key

def require_admin(key: ApiKey = Depends(require_key)) -> ApiKey:
    scopes = getattr(key, "_norm_scopes", _normalize_scopes(key.scopes))
